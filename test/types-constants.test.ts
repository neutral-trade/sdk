...
describe('types and Constants Validation', () => {
  describe('supportedToken enum', () => {
    ...
    it('should have supportedTokenList containing all tokens', () => {
      expect(supportedTokenList).toContain(SupportedToken.USDC)
      expect(supportedTokenList).toContain(SupportedToken.USDT)
      expect(supportedTokenList).toContain(SupportedToken.SOL)
      expect(supportedTokenList.length).toBeGreaterThanOrEqual(6)
    })
  })

  describe('tokens definition', () => {
    it('should have valid Solana addresses for each token', () => {
      for (const tokenSymbol of supportedTokenList) {
        const token = tokens[tokenSymbol]
        expect(token).toBeDefined()
        expect(token.name).toBeTruthy()
        expect(token.symbol).toBeTruthy()

        const solanaInfo = token.onChain[SupportedChain.Solana]
        if (solanaInfo) {
          // Solana addresses are base58 encoded, 32-44 characters
          expect(solanaInfo.address.length).toBeGreaterThanOrEqual(32)
          expect(solanaInfo.address.length).toBeLessThanOrEqual(44)
          expect(solanaInfo.decimals).toBeGreaterThanOrEqual(0)
          expect(solanaInfo.decimals).toBeLessThanOrEqual(18)
        }
      }
    })
  ...
  describe('getSolanaTokenInfo', () => {
    it('should return correct info for USDC', () => {
      const info = getSolanaTokenInfo(SupportedToken.USDC)
      expect(info).not.toBeNull()
      expect(info?.address).toBe('EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v')
      expect(info?.decimals).toBe(6)
    })

    it('should return correct info for SOL', () => {
      const info = getSolanaTokenInfo(SupportedToken.SOL)
      expect(info).not.toBeNull()
      expect(info?.address).toBe('So11111111111111111111111111111111111111112')
      expect(info?.decimals).toBe(9)
    })

    it('should work for all supported tokens', () => {
      for (const tokenSymbol of supportedTokenList) {
        const info = getSolanaTokenInfo(tokenSymbol)
        // All current tokens should have Solana support
        expect(info).not.toBeNull()
      }
    })
  })
